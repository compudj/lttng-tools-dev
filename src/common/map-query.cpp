		...
		enum lttng_map_query_config_app_bitness bitness)
{
	...
	query = (lttng_map_query *) zmalloc(sizeof(struct lttng_map_query));
	if (!query) {
		goto end;
	}
		...
		struct lttng_map_query **query)
{
	...
	local_query = lttng_map_query_create( (lttng_map_query_config_cpu) query_comm->config_cpu,
			(lttng_map_query_config_buffer) query_comm->config_buffer, (lttng_map_query_config_app_bitness) query_comm->config_app_bitness);
	if (!local_query) {
		ret = -1;
		goto end;
	}
