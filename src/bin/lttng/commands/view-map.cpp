...
int compare_key_value_by_key(const void *a, const void *b)
{
	...
	enum lttng_map_content_status map_status;

	map_status = lttng_map_key_value_pair_get_key(kv_a, &key_a);
	assert(map_status == LTTNG_MAP_CONTENT_STATUS_OK);
		...
		enum lttng_buffer_type buffer_type)
{
	...
	if (map_status != LTTNG_MAP_CONTENT_STATUS_OK) {
	...
	for (i = 0; i < key_value_pair_count; i++) {
		...
		longest_key_len = std::max(longest_key_len, strlen(cur_key));

		/* Keep track of the longest value. */
		lttng_map_key_value_pair_get_value(pair, &cur_val);
		longest_val_len = std::max(longest_val_len, number_of_digit(cur_val));
	}
	...
	if (key_value_pair_count == 0) {
		...
		for (i = 0; i < key_value_pair_count; i++) {
			...
			print_one_map_key_value_pair(
				(lttng_map_key_value_pair *) lttng_dynamic_pointer_array_get_pointer(
		...
		const struct lttng_map *map, const char *key_filter)
{
	...
	enum lttng_map_content_status map_content_status;
	...
	map_content_status= lttng_map_content_get_count(map_content,
			&map_content_section_count);
	if (map_content_status != LTTNG_MAP_CONTENT_STATUS_OK) {
		ret = LTTNG_ERR_MAP_VALUES_LIST_FAIL;
...
int cmd_view_map(int argc, const char **argv)
{
	...
	argpar_iter = argpar_iter_create(argc, argv, view_map_options);
	if (!argpar_iter) {
	...
	while (true) {
		...
		if (argpar_item_type(argpar_item) == ARGPAR_ITEM_TYPE_OPT) {
			...
			case OPT_KERNEL:
				...
				break;
			case OPT_KEY:
				if (!assign_string(&opt_key_filter, arg,
						"--key")) {
					goto error;
				}
				break;
