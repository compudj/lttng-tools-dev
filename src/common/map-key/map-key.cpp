...
struct lttng_map_key *lttng_map_key_create(void)
{
	 ...
	 key = (lttng_map_key *) zmalloc(sizeof(*key));
	 if (!key) {
	 	 return NULL;
	 }
		...
		struct lttng_map_key **out_key)
{
	...
	for (i = 0; i < comm->token_count; i++) {
		...
		{
			const char *str_val;
			const struct lttng_map_key_token_string_comm *str_tok_comm;

			str_tok_comm = (typeof(str_tok_comm)) token_comm;
			str_val = (const char *) &str_tok_comm->payload;

			if (!lttng_buffer_view_contains_string(&child_view.buffer,
					str_val, str_tok_comm->string_len)) {
				ret = -1;
				goto end;
			}
			...
			consumed_len += sizeof(*str_tok_comm) + str_tok_comm->string_len;

			break;
		}
		case LTTNG_MAP_KEY_TOKEN_TYPE_VARIABLE:
		{
			const struct lttng_map_key_token_variable_comm *var_tok_comm;

			var_tok_comm = (typeof(var_tok_comm)) token_comm;
			key_status = lttng_map_key_append_token_variable(key,
					(lttng_map_key_token_variable_type) var_tok_comm->var_type);
			if (key_status != LTTNG_MAP_KEY_STATUS_OK) {
				ret = -1;
				goto end;
			}

			consumed_len += sizeof(*var_tok_comm);
			break;
		}
		default:
			abort();
	...
	for (i = 0; i < nb_tokens; i++) {
		...
		case LTTNG_MAP_KEY_TOKEN_TYPE_STRING:
		{
			struct lttng_map_key_token_string *str_token =
					(struct lttng_map_key_token_string *) token;
			struct lttng_map_key_token_string_comm str_tok_comm = {0};
			uint32_t len = strlen(str_token->string) + 1;

			DBG("Serializing a string type key token");
			str_tok_comm.parent_type = token_type;
			str_tok_comm.string_len = len;
			...
			ret = lttng_dynamic_buffer_append(&payload->buffer,
				&str_tok_comm, sizeof(str_tok_comm));
			if (ret) {
				goto end;
			}
		...
		case LTTNG_MAP_KEY_TOKEN_TYPE_VARIABLE:
		{
			...
			struct lttng_map_key_token_variable_comm var_tok_comm = {0};

			DBG("Serializing a variable type key token");

			var_tok_comm.parent_type = token_type;
			var_tok_comm.var_type = var_token->type;

			ret = lttng_dynamic_buffer_append(&payload->buffer,
					&var_tok_comm, sizeof(var_tok_comm));
			if (ret) {
				goto end;
			}
...
enum lttng_error_code lttng_map_key_mi_serialize(const struct lttng_map_key *key,
		struct mi_writer *writer)
{
	...
	/* Open map key element. */
	ret = mi_lttng_writer_open_element(
			writer, mi_lttng_element_map_key);
	if (ret) {
		goto mi_error;
	}

	for_each_map_key_token_const(token, key) {
		ret = mi_lttng_writer_open_element(
				writer, mi_lttng_element_map_key_token);
		if (ret) {
			goto mi_error;
		}
		DBG("Serializing map key token's type: %d", token->type);
		...
		case LTTNG_MAP_KEY_TOKEN_TYPE_STRING:
		{
			...
			ret = mi_lttng_writer_open_element(writer,
					mi_lttng_element_map_key_token_literal_string);
			if (ret) {
				goto mi_error;
			}

			ret = mi_lttng_writer_write_element_string(
					writer, mi_lttng_element_map_key_token_literal_string_value,
					str_token->string);
			if (ret) {
				goto mi_error;
			}

			/* Close inner <token> tag. */
		...
		case LTTNG_MAP_KEY_TOKEN_TYPE_VARIABLE:
		{
			...
			ret = mi_lttng_writer_open_element(writer,
				mi_lttng_element_map_key_token_variable);
			...
			case LTTNG_MAP_KEY_TOKEN_VARIABLE_TYPE_EVENT_NAME:
				ret = mi_lttng_writer_write_element_string(
					writer, mi_lttng_element_map_key_token_variable_type,
					"EVENT_NAME");
				if (ret) {
					goto mi_error;
				}
		...
		enum lttng_map_key_token_variable_type var_type)
{
	...
	token = (lttng_map_key_token_variable *) zmalloc(sizeof(*token));
	if (!token) {
		...
		struct lttng_map_key *key, const char *string)
{
	...
	token = (lttng_map_key_token_string *)zmalloc(sizeof(*token));
	if (!token) {
		status = LTTNG_MAP_KEY_STATUS_ERROR;
		goto end;
	}
		...
		unsigned int index)
{
	...
	token = (lttng_map_key_token *) lttng_dynamic_pointer_array_get_pointer(&key->tokens,
			index);
