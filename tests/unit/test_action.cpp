...
{
	int ret, action_idx;
	struct lttng_action *list_action = NULL,
			    *list_action_from_buffer = NULL,
			    *mut_inner_action = NULL,
	...
	(void) lttng_action_start_session_set_session_name(start_session_action,
			"une-session");

	stop_session_action = lttng_action_stop_session_create();
	(void) lttng_action_stop_session_set_session_name(stop_session_action,
			"une-autre-session");
	notify_action = lttng_action_notify_create();
...

	{
		struct lttng_payload_view view =
				lttng_payload_view_from_payload(
						&payload, 0, -1);
		(void) lttng_action_create_from_payload(
				&view, &list_action_from_buffer);
	}
	ok(list_action_from_buffer,
			"Notify action created from payload is non-null");
	...
	action_idx = 0;
	for_each_actions_const(const_inner_action, list_action) {
	...
	action_idx = 0;
	for_each_actions_mutable(mut_inner_action, list_action) {
		enum lttng_action_type inner_action_type =
