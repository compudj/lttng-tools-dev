...
static
bool relayd_supports_chunks(struct lttcomm_relayd_sock *sock)
{
	if (sock->major > 2) {
		return true;
...
static int relayd_create_session_2_11(struct lttcomm_relayd_sock *rsock,
		...
		time_t creation_time, bool session_name_has_creation_time,
		...
		char *output_path)
{
	...
	msg->session_name_has_creation_time = session_name_has_creation_time;
	if (current_chunk_id) {
	...
	if (reply->output_path_length >= PATH_MAX) {
		ERR("output path length too long");
		ret = -1;
		goto error;
	}
		...
		const struct relayd_stream_rotation_position *positions)
{
	...
	if (!relayd_supports_chunks(sock)) {
		DBG("Relayd does not support chunks");
		return 0;
	}
