		...
		bool *session_name_has_creation_time)
{
	...
	if (header.base_path_len > PATH_MAX) {
		ret = -ENAMETOOLONG;
	...
	if (base_path_view.data[base_path_view.size - 1] != '\0') {
	...
	strcpy(base_path, base_path_view.data);
	...
	*session_name_has_creation_time =
		header.session_name_has_creation_time;
