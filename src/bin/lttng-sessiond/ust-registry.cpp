...
{
	int ret;
	char _key[LTTNG_UST_ABI_KEY_TOKEN_STRING_LEN_MAX] = {0};
		...
		struct lttng_map_key *key)
{
	...
	key_entry = (ust_registry_map_key_ht_entry *) zmalloc(sizeof(struct ust_registry_map_key_ht_entry));
	if (!key_entry) {
		ret = -ENOMEM;
		goto end;
	}
		...
		const char *formated_key, uint64_t *index)
{
	...
	if (index_node) {
		...
		*index = index_entry->index;
		 ...
		 */
		index_entry = (ust_registry_map_index_ht_entry *) zmalloc(sizeof(struct ust_registry_map_index_ht_entry));
		if (!index_entry) {
			ret = -1;
			goto end;
		}
		...
		struct ust_app *app)
{
	...
	*counter_index_p = counter_index;
	...
	DBG3("UST registry allocating counter index %" PRIu64 " to event: %s, "
		"sess_objd: %u, map_objd: %u, map_id: %u",
		counter_index, name, session_objd, map_objd, map->map_id);
...
void destroy_map_rcu(struct rcu_head *head)
{
	...
	if (map->events_ht) {
		lttng_ht_destroy(map->events_ht);
	}

	if (map->tracer_token_to_map_key_ht) {
		lttng_ht_destroy(map->tracer_token_to_map_key_ht);
	}

	if (map->key_string_to_bucket_index_ht) {
		lttng_ht_destroy(map->key_string_to_bucket_index_ht);
	}
		...
		uint64_t key)
{
	...
	map = (ust_registry_map *) zmalloc(sizeof(*map));
	if (!map) {
...
void ust_registry_session_destroy(struct ust_registry_session *reg)
{
	...
	if (reg->maps) {
		rcu_read_lock();
		/* Destroy all event associated with this registry. */
		cds_lfht_for_each_entry(reg->maps->ht, &iter.iter, map,
				node.node) {
			/* Delete the node from the ht and free it. */
			ret = lttng_ht_del(reg->maps, &iter);
			assert(!ret);
			destroy_map(map);
		}
		rcu_read_unlock();
		lttng_ht_destroy(reg->maps);
	}
