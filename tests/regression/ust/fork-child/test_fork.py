...
tmp_dir_sync_after_fork_child = tempfile.mkdtemp()
...
#TODO: stdout, stderr can fill buffer and deadlock.
fork_process = subprocess.Popen([test_path + "fork", test_path + "fork2", tmp_dir_sync_after_fork_child + "/after_fork_child", tmp_dir_sync_after_fork_child + "/after_tracing_start"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
open(tmp_dir_sync_after_fork_child + "/after_tracing_start", 'w+').close()
...
for event_line in event_lines:
    ...
    if re.search(r"after_child_wait_parent", event_line):
        event_after_child_wait_parent = (event_pid == parent_pid)
...
print_test_result(event_after_child_wait_parent, current_test, "after_child_wait_parent event logged by parent process found in trace")
current_test += 1
...
shutil.rmtree(tmp_dir_sync_after_fork_child)
