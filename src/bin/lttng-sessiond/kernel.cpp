...
enum lttng_error_code kernel_map_clear_all(struct ltt_kernel_map *map)
{
	...
	for(i = 0; i < descr_count; i++) {
		...
		struct lttng_kernel_abi_counter_clear counter_clear = {0};
...
enum lttng_error_code kernel_clear_session_maps(struct ltt_session *session)
{
	...
	cds_list_for_each_entry(map, &ksess->map_list.head, list) {
		DBG("Clear kernel map %s, session %s",
				map->map->name, session->name);
...
static
int add_key_token(struct lttng_kernel_abi_key_token *kernel_key_token,
		...
		struct lttng_map_key *key)
{
	...
	struct lttng_kernel_abi_counter_event k_counter_event = {};


	event_counter = (ltt_kernel_event_counter *) zmalloc(sizeof(*event_counter));
	if (!event_counter) {
