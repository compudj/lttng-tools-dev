		...
		struct lttng_map **map_out)
{
	...
	map = (lttng_map *)zmalloc(sizeof(struct lttng_map));
	if (!map) {
		status = LTTNG_MAP_STATUS_ERROR;
		goto end;
	}
	...
	// JORAJ FIXME: prime containder for c++ data structure
	map->dimension_count = dimension_count;
	map->dimension_sizes = (uint64_t *)zmalloc(
		...
		struct lttng_map **map)
{
	...
	domain = (lttng_domain_type) map_comm->domain;
	buffer_type = (lttng_buffer_type) map_comm->buffer_type;
	bitness = (lttng_map_bitness) map_comm->bitness;
	boundary_policy = (lttng_map_boundary_policy) map_comm->boundary_policy;
...

	{
		...
		dimension_sizes = (uint64_t *) zmalloc(map_dim_sizes_len);
		if (!dimension_sizes) {
			ret = -1;
			goto end;
		}
...
{
	struct lttng_map *map = (lttng_map *)ptr;
...
struct lttng_map_list *lttng_map_list_create(void)
{
	...
	map_list = (lttng_map_list *) zmalloc(sizeof(*map_list));
	if (!map_list) {
		goto end;
	}
