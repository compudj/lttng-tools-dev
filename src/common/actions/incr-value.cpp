...
static enum lttng_error_code lttng_action_incr_value_mi_serialize(
		const struct lttng_action *action, struct mi_writer *writer)
{
	int ret;
	enum lttng_error_code ret_code;
	enum lttng_action_status status;
	const char *session_name = NULL, *map_name = NULL;
	const struct lttng_map_key *key = NULL;

	assert(action);
	assert(IS_INCR_VALUE_ACTION(action));

	status = lttng_action_incr_value_get_session_name(
			action, &session_name);
	assert(status == LTTNG_ACTION_STATUS_OK);
	assert(session_name != NULL);

	status = lttng_action_incr_value_get_map_name(
			action, &map_name);
	assert(status == LTTNG_ACTION_STATUS_OK);
	assert(map_name != NULL);

	status = lttng_action_incr_value_get_key(action, &key);
	assert(status == LTTNG_ACTION_STATUS_OK);
	assert(key != NULL);

	/* Open action incr-value action element. */
	ret = mi_lttng_writer_open_element(writer,
			mi_lttng_element_action_incr_value);
	if (ret) {
		goto mi_error;
	}
	...
	ret = mi_lttng_writer_write_element_string(
			writer, mi_lttng_element_map_name, map_name);
	if (ret) {
		goto mi_error;
	}

	/* Key */
	ret_code = lttng_map_key_mi_serialize(key, writer);
	if (ret_code != LTTNG_OK) {
		goto end;
	}

	/* Close action incr-value element. */
...
{
	struct lttng_action_incr_value *incr_value;

	incr_value = (lttng_action_incr_value *) zmalloc(sizeof(struct lttng_action_incr_value));
	if (!incr_value) {
		goto end;
	}

	lttng_action_init(&incr_value->parent, LTTNG_ACTION_TYPE_INCREMENT_VALUE,
			...
			lttng_action_incr_value_mi_serialize);

end:
	return &incr_value->parent;
}
