...
function test_map_view_empty()
{
	...
	local map_domain
	local incr_value_domain

	if [ "$param_domain" = "user" ]; then
		map_domain="--userspace"
		incr_value_domain="user"
	else
		map_domain="--kernel"
		incr_value_domain="kernel"
	fi

	diag "Map view empty: $param_domain bitness $bitness $buf_option"

	create_lttng_session_ok "$SESSION_NAME"

	lttng_add_map_ok "$MAP_NAME" "$SESSION_NAME" "$map_domain" "$bitness" "$buf_option"

	"$FULL_LTTNG_BIN" view-map "$MAP_NAME" > /dev/null
	ok $? "Map enabled viewed succesfully"

	"$FULL_LTTNG_BIN" disable-map "$map_domain" "$MAP_NAME" > /dev/null
...
function test_map_formated_keys()
{
	...
	local bitness="32"
	# buf option left empty for use with both UST and kernel param_domain.
	local buf_option=""
	local MAP_NAME="my_map_name"
	local SESSION_NAME="my_session_name"
	...
	diag "Map with $map_domain formated key. event-name: \"$event_name\", key format: \"$key_format\", expecting: \"$expected_key\""
	...
	lttng_add_trigger_ok "$TRIGGER_NAME" \
		--condition \
			event-rule-matches --type="$incr_value_domain" --name="$event_name" \
		--action \
...
function test_map_n_triggers_n_keys()
{
	...
	diag "Map $map_domain with $number_of_trigger triggers with all different keys"
...
function test_map_n_triggers_1_key()
{
	...
	diag "Map $map_domain with $number_of_trigger triggers all with the same key"
...
function test_map_n_triggers_1_key_coalesced()
{
	...
	lttng_add_map_ok "$MAP_NAME" "$SESSION_NAME" "$map_domain" "$bitness" "$buf_option" "--coalesce-hits"
...
function test_map_disable_enable()
{
	...
	diag "Map $map_domain disable-enable --bitness $bitness"
	...
	"$FULL_LTTNG_BIN" disable-map "$map_domain" -s "$SESSION_NAME" "$MAP_NAME" > /dev/null
	ok $? "Map disabled succesfully"
	...
	"$FULL_LTTNG_BIN" enable-map "$map_domain" -s "$SESSION_NAME" "$MAP_NAME" > /dev/null
...
function test_map_add_remove_add_trigger()
{
	...
	diag "Map $map_domain add-remove-add the same trigger"
...
function test_map_creation_after_trigger()
{
	...
	diag "Map $map_domain creation after trigger creation"
...
function test_map_filter()
{
	...
	diag "Map $map_domain filtering $event_name filter: \"$filter_field == 0\""
	...
	lttng_add_trigger_ok "$TRIGGER_NAME" \
		--condition \
			event-rule-matches --type="$incr_value_domain" --name="$event_name" --filter "$filter_field==0"\
		--action \
			incr-value --session "$SESSION_NAME" --map "$MAP_NAME" --key "$KEY"
