...
static int session_set_anonymous_chunk(struct relay_session *session)
{
	...
	char *output_path = opt_output_path;
	bool output_path_allocated = false;

	if (output_path == NULL) {
		/* No output path defined */
		output_path = utils_get_home_dir();
		if (output_path == NULL) {
		...
		ret = asprintf(&output_path, "%s/%s", output_path, DEFAULT_TRACE_DIR_NAME);
		if (ret < 0) {
		...
		bool session_name_has_creation_time)
{
	...
	session->session_name_has_creation_time =
