		...
		struct consumer_output *consumer)
{
	...
	if (uri->stype == LTTNG_STREAM_CONTROL) {
		...
		if (result_flags & LTTCOMM_RELAYD_CONFIGURATION_FLAG_CLEAR_ALLOWED) {
			consumer->relay_clear_feature = true;
		}
...
int cmd_setup_relayd(struct ltt_session *session)
{
	...
	if (usess && usess->consumer && usess->consumer->type == CONSUMER_DST_NET
		...
		session->consumer->relay_clear_feature =
			usess->consumer->relay_clear_feature;
	}
	...
	if (ksess && ksess->consumer && ksess->consumer->type == CONSUMER_DST_NET
		...
		session->consumer->relay_clear_feature =
			ksess->consumer->relay_clear_feature;
	}
...
int cmd_stop_trace(struct ltt_session *session)
{
	...
	DBG("Session stopped %s (id %" PRIu64 ")", session->name, session->id);
